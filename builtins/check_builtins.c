...
/*   Updated: 2025/08/26 19:46:20 by jkroger          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (!ft_strncmp(cmd_lst->s_exec.cmd_split[0], "exit", 5))
	{
...
{
	if (!ft_strncmp(cmd_lst->s_exec.cmd_split[0], "unset", 6))
	{
...
{
	get_signals_child();
	if (!is_builtin(cmd_lst->s_exec.cmd_split[0]) && cmd_lst->err != 0)
	{
		set_err(cmd_lst->err_file, cmd_lst->err);
		return (1);
	}
	if (!ft_strncmp(cmd_lst->s_exec.cmd_split[0], "cd", 3))
	{
		cd(cmd_lst, environ);
		return (1);
	}
	else if (!ft_strncmp(cmd_lst->s_exec.cmd_split[0], "export", 7))
	{
