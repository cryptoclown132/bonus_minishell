...
void	builtin_exit(t_cmd_tree *cmd_struct, t_env_var *environ)
{
	...
	if (cmd_struct->s_exec.cmd_split[1]
		&& !check_numeric(cmd_struct->s_exec.cmd_split[1]))
	{
		ft_putstr_fd("shell: exit: numeric argument required\n", 2);
		g_exit_status = 255;
	}
	else if (cmd_struct->s_exec.cmd_split[1] && cmd_struct->s_exec.cmd_split[2])
	{
		ft_putstr_fd("shell: exit: too many arguments\n", 2);
		g_exit_status = 1;
		return ;
	}
	else if (cmd_struct->s_exec.cmd_split[1]
		&& check_numeric(cmd_struct->s_exec.cmd_split[1]))
		g_exit_status = ft_atoi(cmd_struct->s_exec.cmd_split[1]);
	else
		g_exit_status = 0;
